				       ...
				       const uint8_t value)
{
	...
	if ((value < led_datas->min_brightness) || (value > led_datas->max_brightness)) {
		err = -EINVAL;
...
static int tlc59731_led_blink(const struct device *dev, uint32_t led, uint32_t delay_on,
				  uint32_t delay_off)
{
	int32_t err = 0;
	const struct tlc59731_cfg *tlc_conf = dev->config;
	...
	if ((led_datas->min_period) && (led_datas->max_period)) {
		if ((delay_on < led_datas->min_period) || (delay_on > led_datas->max_period)) {
			err = -EINVAL;
		...
		if ((delay_off < led_datas->min_period) || (delay_off > led_datas->max_period)) {
			err = -EINVAL;
...
static int tlc59731_led_init(const struct device *dev)
{
	...
	LOG_ERR("%s: Min/Max Brightness: %d/%d. Min/Max Period: %d/%d", dev->name,
		led_datas->min_brightness, led_datas->max_brightness, led_datas->min_period,
