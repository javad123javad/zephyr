...
#define DT_DRV_COMPAT u_blox_6m
...
struct ubx_m8_data {
    ...
    struct gnss_satellite satellites[CONFIG_GNSS_U_BLOX_6M_SATELLITES_COUNT];
#endif
};
...
UBX_FRAME_ARRAY_DEFINE(u_blox_m8_init_seq,
                       &disable_gga, &disable_gns, &disable_rmc, &disable_gsv, &disable_dtm, &disable_gbs,
                       &disable_gll, &disable_grs, &disable_gsa, &disable_gst,
                          ...
                          size_t len, void *rsp, size_t min_rsp_size)
{
    ...
    data->script.inst.timeout = K_SECONDS(8);
