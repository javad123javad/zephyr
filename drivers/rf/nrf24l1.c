...
static uint8_t nrf24l01_write_register(const struct device *dev, uint8_t reg, uint8_t data)
{
	...
	uint8_t rx_data = 0;
	int ret;
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = 2}};
	const struct spi_buf rx_buf[1] = {{.buf = &rx_data, .len = 2}};
...
static uint8_t nrf24l01_read_register(const struct device *dev, uint8_t reg)
{
	const struct nrf24l01_config *config = dev->config;
	uint8_t tx_data[2];
	uint8_t rx_data[2];
	int ret;
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = 2}};
	const struct spi_buf rx_buf[1] = {{.buf = (void *)rx_data, .len = 2}};
	struct spi_buf_set tx = {.buffers = tx_buf, .count = 1};
	const struct spi_buf_set rx = {.buffers = rx_buf, .count = 1};
...
static uint8_t nrf24l01_cmd_register(const struct device *dev, uint8_t cmd)
{
	const struct nrf24l01_config *config = dev->config;
	int ret;
	uint8_t tx_data[1];
	uint8_t rx_data[1];
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = 1}};
	const struct spi_buf rx_buf[1] = {{.buf = rx_data, .len = 1}};
	struct spi_buf_set tx = {.buffers = tx_buf, .count = 1};
	const struct spi_buf_set rx = {.buffers = rx_buf, .count = 1};
...
static uint8_t nrf24l01_cmd_register_with_arg(const struct device *dev, uint8_t cmd, uint8_t arg)
{
	const struct nrf24l01_config *config = dev->config;
	int ret;
	uint8_t tx_data[2];
...
static uint8_t nrf24l01_activate(const struct device *dev)
{
 ...
 */
static uint8_t nrf24l01_clear_irq(const struct device *dev)
{
	uint8_t reg;
...
static uint8_t nrf24l01_set_register_bit(const struct device *dev, uint8_t reg, uint8_t bit,
					 bool val)
{
...
static bool nrf24l01_get_register_bit(const struct device *dev, uint8_t reg, uint8_t bit)
{
	uint8_t reg_data;
	bool res;
...
static uint8_t nrf24l01_write_register_len(const struct device *dev, uint8_t reg,
					   const uint8_t *data, uint8_t len)
{
	...
	uint8_t rx_data = 0;
	int ret;
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = (len + 1)}};
	const struct spi_buf rx_buf[1] = {{.buf = &rx_data, .len = 1}};
	struct spi_buf_set tx = {.buffers = tx_buf, .count = 1};
	const struct spi_buf_set rx = {.buffers = rx_buf, .count = 1};
...
static int nrf24l01_toggle_ce(const struct device *dev, bool level)
{
	const struct nrf24l01_config *config = dev->config;
	int ret;
...
static uint8_t nrf24l01_set_config(const struct device *dev)
{
	uint8_t val;
	struct nrf24l01_data *data = dev->data;
...
static uint8_t nrf24l01_get_channel(const struct device *dev)
{
	return nrf24l01_read_register(dev, RF_CH);
}

static uint8_t nrf24l01_get_config(const struct device *dev)
{
	return nrf24l01_read_register(dev, NRF_CONFIG);
}

static uint8_t nrf24l01_write_payload_core(const struct device *dev, const void *buf,
					   uint8_t data_len, const uint8_t write_type)
{
	int ret;
	/* Can be TX or RX ACK*/
	struct nrf24l01_data *data = dev->data;
	...
	uint8_t rx_data[1];
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = size}};
	const struct spi_buf rx_buf[1] = {{.buf = rx_data, .len = 1}};
	struct spi_buf_set tx = {.buffers = tx_buf, .count = 1};
	const struct spi_buf_set rx = {.buffers = rx_buf, .count = 1};
...
static uint8_t nrf24l01_write_tx_payload(const struct device *dev, const void *buf,
					 uint8_t data_len)
{
	uint8_t ret, cmd;
	struct nrf24l01_data *data = dev->data;
...
static uint8_t nrf24l01_read_payload(const struct device *dev, void *buf, uint8_t data_len)
{
	/* In RX mode only*/
	struct nrf24l01_data *data = dev->data;
	const struct nrf24l01_config *config = dev->config;
	...
	const struct spi_buf tx_buf[1] = {{.buf = tx_data, .len = size}};
	const struct spi_buf rx_buf[1] = {{.buf = rx_data, .len = size}};
	struct spi_buf_set tx = {.buffers = tx_buf, .count = 1};
	const struct spi_buf_set rx = {.buffers = rx_buf, .count = 1};
...
static inline void nrf24l01_toggle_reading_pipe(const struct device *dev, uint8_t rx_pipe,
						bool activate)
{
	nrf24l01_set_register_bit(dev, EN_RXADDR, child_pipe_enable_bit[rx_pipe], activate);
}

static void nrf24l01_configure_pipes(const struct device *dev)
{
	struct nrf24l01_data *data = dev->data;
	int idx;
	...
	for (idx = 0; idx < 4; idx++) {
		nrf24l01_write_register(dev, RX_ADDR_P2 + idx,
					data->rx_child_datapipes_addresses[idx]);
	}
...
static void nrf24l01_configure_ack(const struct device *dev)
{
	struct nrf24l01_data *data = dev->data;
	int idx;

	for (idx = 0; idx < data->rx_datapipes_number; idx++) {
		nrf24l01_set_register_bit(dev, DYNPD, DPL_P0 + idx,
					  data->rx_datapipes_dynamic_payload[idx]);
	}
	...
	if (data->payload_ack) {
		if (memcmp(data->rx_datapipe0_address, data->tx_address, data->addr_width)) {
			LOG_WRN("RX datapipe 0 and TX address are different, use this config only "
				"for RX mode");
		}
	}
}

static bool nrf24l01_is_rx_data_available(const struct device *dev, uint8_t *pipe_num)
{
...
static void nrf24l01_radio_power_up(const struct device *dev)
{
	// if not powered up then power up and wait for the radio to initialize
	if (!nrf24l01_get_register_bit(dev, NRF_CONFIG, PWR_UP)) {
		nrf24l01_set_register_bit(dev, NRF_CONFIG, PWR_UP, true);

		// For nRF24L01+ to go from power down mode to TX or RX mode it must first pass
		// through stand-by mode. There must be a delay of Tpd2stby (see Table 16.) after
		// the nRF24L01+ leaves power down mode before the CE is set high. - Tpd2stby can be
		// up to 5ms per the 1.0 datasheet
		k_sleep(K_MSEC(5));
	}
}
...
static void nrf24l01_stop_listening(const struct device *dev)
{
	struct nrf24l01_data *data = dev->data;
...
static bool nrf24l01_test_spi(const struct device *dev)
{
	uint8_t ret = 0;
...
static void nrf24l01_print_status(uint8_t status)
{
	LOG_INF("STATUS = 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x", status,
		(status & BIT(RX_DR)) ? 1 : 0, (status & BIT(TX_DS)) ? 1 : 0,
		(status & BIT(MAX_RT)) ? 1 : 0, ((status >> RX_P_NO) & 0x07),
...
void irq_callback_handler(const struct device *port, struct gpio_callback *cb, uint32_t pins)
{
...
{
	struct nrf24l01_data *data = CONTAINER_OF(item, struct nrf24l01_data, trig_work);
	...
	if (ret & BIT(RX_DR)) {
		...
		if (data->is_listening) { // Not an ACK interrupt
...
//////////////////////////
int nrf24l01_config(const struct device *dev, struct rf_modem_config *config)
{
	return 0;
}

int nrf24l01_send_async(const struct device *dev, uint8_t *buffer, uint32_t data_len,
...
static const struct rf_driver_api nrf24l01_api = {
	.config = nrf24l01_config,
	.send = nrf24l01_write,
	.send_async = nrf24l01_send_async,
	.recv = nrf24l01_read,
	.recv_async = nrf24l01_recv_async,
};
...
#define NRF24L01_DEVICE_INIT(n)                                                                    \
	static struct nrf24l01_data dev_data_##n = {                                               \
		...
		.is_listening = false,                                                             \
		.write_ret_code = 0,                                                               \
		.rx_child_datapipes_addresses = {DT_INST_PROP(n, rx_datapipe2_address),            \
		...
		.rx_datapipes_dynamic_payload =                                                    \
			DT_INST_PROP_OR(n, rx_datapipes_dynamic_payload, {}),                      \
	};                                                                                         \
	...
	DEVICE_DT_INST_DEFINE(n, &nrf24l01_init, NULL, &dev_data_##n, &dev_config_##n,             \
			      POST_KERNEL, CONFIG_RF_INIT_PRIORITY, &nrf24l01_api);
