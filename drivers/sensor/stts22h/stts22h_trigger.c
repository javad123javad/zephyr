...
static void stts22h_handle_interrupt(const struct device *dev)
{
	...
	//stts22h_status_reg_get(stts22h->ctx, &status);
	stts22h_dev_status_get(stts22h->ctx, &status);

	if (stts22h->thsld_handler != NULL &&
	    (status.over_thh || status.under_thl)) {
...
int stts22h_init_interrupt(const struct device *dev)
{
	...
	stts22h_high_temperature_threshold_set(stts22h->ctx,
					stts22h_from_celsius_to_lsb(temp_hi));

	stts22h_low_temperature_threshold_set(stts22h->ctx,
					stts22h_from_celsius_to_lsb(temp_lo));
